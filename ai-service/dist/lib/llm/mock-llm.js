 ...
 */
export async function mockChatCompletion(systemPrompt, userPrompt, _history = [], additionalContext = []) {
    ...
    if (/fix|resolved|last time/i.test(q)) {
        if (!topIssue)
            return 'No matching resolutions in the knowledge base yet. Seed demo data or sync from Jira.';
        return [
    ...
    if (/summarize|summary/i.test(q)) {
        return [
            ...
            '',
            ...issues.slice(0, 3).map((iss, i) => `**${i + 1}. ${iss.id}** — ${iss.metadata?.title || 'Related ticket'}\n${iss.metadata?.resolution?.slice(0, 120) || iss.content.slice(0, 120)}...`),
            '',
