...
function formatGitlabError(err) {
    if (axios.isAxiosError(err)) {
        ...
        if (status === 401) {
            return ('GitLab returned 401 Unauthorized. Regenerate GITLAB_TOKEN with "api" scope at ' +
        ...
        if (status)
            return `GitLab API error (${status}): ${msg}`;
        if (ax.code === 'ECONNABORTED')
            return 'GitLab API timed out.';
        if (ax.code === 'ECONNRESET' || ax.message.toLowerCase().includes('econnreset')) {
            return ('GitLab closed the connection (ECONNRESET). Retry in a few seconds. ' +
...
export async function validateGitlabToken() {
    ...
    try {
        const { data, status } = await axios.get(`${config.gitlabUrl}/api/v4/user`, { headers: gitlabHeaders(), timeout: 15000, validateStatus: () => true });
...
function buildFullPrompt(systemPrompt, userPrompt, history, additionalContext, opts) {
 ...
 */
export async function gitlabChatCompletion(systemPrompt, userPrompt, history = [], additionalContext = [], options = {}) {
    if (!config.gitlabUrl || !config.gitlabToken) {
    ...
    const useGraphql = config.gitlabUrl.includes('gitlab.com') ||
    ...
    try {
        const { data, status } = await withRetry(() => axios.post(url, {
            content: fullContent,
        ...
        }, { headers: gitlabHeaders(), timeout: 90000, validateStatus: () => true }), { label: 'gitlab-rest-chat', attempts: 2, delayMs: 2000 });
        if (status === 404) {
            return await gitlabChatViaGraphql(fullContent, graphqlTag, {
                maxAttempts: options.graphqlMaxAttempts,
                securityMode: options.securityMode,
...
function extractRestChatBody(data) {
 ...
 */
export async function gitlabSecurityCompletion(systemPrompt, filePath, codeSnippet) {
    if (!config.gitlabUrl || !config.gitlabToken)
        return null;
    const setup = await getGitlabSetupStatusAsync();
    if (!setup.ready)
    ...
    try {
        ...
        console.warn(`[gitlab-security] GraphQL failed for ${filePath}:`, err instanceof Error ? err.message : err);
        return restBody;
    }
}
