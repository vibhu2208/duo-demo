import { chatCompletion } from '../lib/llm/index.js';
import { searchSimilar } from '../lib/chroma.js';
import { buildTicketDocument } from '../lib/text.js';
import { MIN_SIMILARITY, formatChatAnswer, greetingReply, isGreeting, isSubstantiveQuestion, } from '../lib/chat-utils.js';
...
export async function generateRecommendation(payload) {
    const context = payload.similarTickets
        .map((t, i) => `[${i + 1}] ${t.jiraKey} (similarity: ${(t.similarityScore * 100).toFixed(1)}%)\nSummary: ${t.summary}\nResolution: ${t.resolutionSummary}`)
        .join('\n\n');
...
Respond ONLY with valid JSON:
{
  ...
  "confidenceScore": 0.0 to 1.0
    ...
    const gitlabContext = payload.similarTickets.map((t) => ({
        category: 'issue',
        id: t.jiraKey,
        content: `${t.summary}\nResolution: ${t.resolutionSummary}`,
...
export async function ragChat(payload) {
    ...
    if (!isSubstantiveQuestion(message) && retrieved.length === 0) {
        return {
            answer: 'I need a bit more detail to search historical tickets. Try asking about a specific error, ticket key, or symptom — e.g. *"Have we seen API timeouts before?"*',
            sources: [],
        };
    }
    const hasDbContext = !!(payload.dbContext && payload.dbContext.length > 100);
    const dbStats = payload.dbStats;
    const vectorBlock = retrieved.length > 0
        ? retrieved
            .map((r) => `--- ${r.jiraKey} (${(r.similarityScore * 100).toFixed(0)}% similar) ---\n${r.summary}\nResolution: ${r.resolutionSummary || 'N/A'}`)
            .join('\n\n')
        : '';
    const contextBlock = [
        dbStats
        ...
        .filter(Boolean)
        .join('\n\n');
    const gitlabContext = retrieved.map((r) => ({
        category: 'issue',
        id: r.jiraKey,
