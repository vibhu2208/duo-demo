...
async function isChromaReachable(): Promise<boolean> {
  if (!config.useChroma) return false;
  if (chromaAvailable !== null) return chromaAvailable;
  try {
    if (!client) client = new ChromaClient({ path: config.chromaUrl });
...
export async function searchSimilar(payload: {
  ...
  try {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      if (payload.excludeTicketId && ticketId === payload.excludeTicketId) continue;
...
function extractResolutionFromDoc(doc: string): string {
