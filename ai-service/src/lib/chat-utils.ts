...
export function formatChatAnswer(raw: string): string {
  ...
  if (jsonMatch) {
    try {
      ...
      const steps = Array.isArray(j.recommendedSteps) ? (j.recommendedSteps as string[]) : [];
      const checklist = Array.isArray(j.investigationChecklist)
        ? (j.investigationChecklist as string[])
        : [];
      const fixes = Array.isArray(j.possibleFixes) ? (j.possibleFixes as string[]) : [];
      const confidence =
        typeof j.confidenceScore === 'number'
...
export function greetingReply(dbStats?: { totalTickets: number }): string {
  const countNote =
    dbStats && dbStats.totalTickets > 0
