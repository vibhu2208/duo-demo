...
function isDuoRefusal(raw: string): boolean {
...
function normalizeFinding(raw: Record<string, unknown>, defaultPath?: string): SecurityFinding | null {
...
function truncateForDuo(content: string, maxChars: number): string {
...
function patternFindingsForFile(file: CodeFileInput): SecurityFinding[] {
...
function mockSecurityReview(files: CodeFileInput[], repo: string): CodeReviewResult {
...
function batchFiles(files: CodeFileInput[], limit: number): CodeFileInput[][] {
  const batches: CodeFileInput[][] = [];
  let current: CodeFileInput[] = [];
  let currentSize = 0;
...
async function callLlmForReview(systemPrompt: string, user: string): Promise<string> {
...
function findingsFromJson(json: Record<string, unknown>, defaultPath?: string): SecurityFinding[] {
  const rawFindings = Array.isArray(json.findings) ? json.findings : [];
  const findings: SecurityFinding[] = [];
  for (const item of rawFindings) {
    if (item && typeof item === 'object') {
      const normalized = normalizeFinding(item as Record<string, unknown>, defaultPath);
...
function findingsFromProse(prose: string, filePath: string): SecurityFinding[] {
  const findings: SecurityFinding[] = [];
  ...
  for (const line of lines) {
    ...
    if (/no (security )?issues|no vulnerabilities|looks good|lgtm/i.test(bullet)) continue;
...
async function reviewSingleFileWithDuo(
  file: CodeFileInput,
  _repo: string,
  _branch: string
): Promise<{ findings: SecurityFinding[]; summary: string; usedDuo: boolean }> {
  ...
  try {
    ...
    console.warn(
      `[security/review] Duo JSON pass failed for ${file.path}:`,
  ...
  try {
    ...
    raw = await duoTaskCompletion(
      'You are an application security engineer reviewing source code.',
      nlPrompt,
      `security-chat:${file.path}`,
      { graphqlMaxAttempts: 40 }
    );
    if (raw && !isDuoRefusal(raw)) {
      const json = parseJsonFromLlm(raw);
      if (json) {
    ...
    console.warn(
      `[security/review] Duo chat-style pass failed for ${file.path}:`,
      err instanceof Error ? err.message : err
    );
  }
...
function splitFileIntoSections(
  ...
  while (lineIdx < lines.length && sections.length < maxSections) {
    ...
    while (lineIdx < lines.length) {
      ...
      if (chunk.length + next.length + 1 > maxChars && chunk.length > 0) break;
      chunk += (chunk ? '\n' : '') + next;
      lineIdx++;
...
function offsetFindingLines(
  finding: SecurityFinding,
  section: FileSection
): SecurityFinding {
  const lineStart =
    finding.lineStart != null ? section.lineStart + finding.lineStart - 1 : section.lineStart;
  const lineEnd =
    finding.lineEnd != null ? section.lineStart + finding.lineEnd - 1 : section.lineEnd;
...
async function reviewSectionWithDuo(
  file: CodeFileInput,
  section: FileSection,
  _repo: string,
  _branch: string
  ...
  try {
    ...
    console.warn(
      `[security/review] Duo section pass failed for ${file.path}#${section.sectionIndex}:`,
      err instanceof Error ? err.message : err
    );
  }
  ...
  try {
    ...
    raw = await duoTaskCompletion(
      'You are an application security engineer reviewing source code.',
      nlPrompt,
      `security-section-chat:${file.path}:${section.sectionIndex}`,
      { graphqlMaxAttempts: 40 }
    );
    if (raw && !isDuoRefusal(raw)) {
      const json = parseJsonFromLlm(raw);
    ...
    console.warn(
      `[security/review] Duo section chat failed for ${file.path}#${section.sectionIndex}:`,
      err instanceof Error ? err.message : err
    );
  }

  const sectionFile: CodeFileInput = { ...file, content: section.content };
...
async function reviewSingleFileInSections(
  file: CodeFileInput,
  repo: string,
  branch: string
): Promise<CodeReviewResult> {
