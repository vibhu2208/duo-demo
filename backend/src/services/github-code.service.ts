...
export interface GitHubCodeConfig {
  token: string;
  defaultOwner?: string;
}
...
export function getGitHubCodeConfigFromEnv(): GitHubCodeConfig | null {
...
async function getGitHubCodeConfigFromDb(userId: string): Promise<GitHubCodeConfig | null> {
  const { rows } = await query<{
    token_encrypted: string;
    default_owner: string | null;
  }>(
    'SELECT token_encrypted, default_owner FROM github_code_config WHERE user_id = $1 LIMIT 1',
    [userId]
  );
...
export async function getGitHubCodeConfigForUser(userId: string): Promise<GitHubCodeConfig | null> {
  const db = await getGitHubCodeConfigFromDb(userId);
  if (db) return db;
  return getGitHubCodeConfigFromEnv();
}

export async function saveGitHubCodeConfig(
  userId: string,
  data: { token: string; defaultOwner?: string }
) {
  await query(
    `INSERT INTO github_code_config (user_id, token_encrypted, default_owner)
     VALUES ($1, $2, $3)
...
export function createGitHubClient(cfg: GitHubCodeConfig): AxiosInstance {
  return axios.create({
...
export async function testGitHubCodeConnection(
  ...
  try {
    ...
    const { data: user, status } = await client.get<{ login: string; name: string | null }>('/user');
...
export async function listAccessibleRepos(cfg: GitHubCodeConfig): Promise<GitHubRepo[]> {
  const client = createGitHubClient(cfg);
...
export async function fetchRepoTree(
  ...
  const { data: branchData } = await client.get<{ commit: { sha: string } }>(
    `/repos/${owner}/${repo}/branches/${encodeURIComponent(branch)}`
  );
  const sha = branchData.commit.sha;

  const { data: tree } = await client.get<{
    tree: { path: string; type: string; size?: number }[];
    truncated: boolean;
  }>(`/repos/${owner}/${repo}/git/trees/${sha}`, { params: { recursive: '1' } });
...
export async function runSecurityScan(
  userId: string,
  params: { repoFullName: string; branch?: string; filePath: string }
  ...
  await query(
    `UPDATE github_code_config SET scan_status = 'running', updated_at = NOW() WHERE user_id = $1`,
    [userId]
  );
  ...
  try {
    ...
    await query(
      `UPDATE github_code_config SET scan_status = 'idle', last_scan_at = NOW(), updated_at = NOW() WHERE user_id = $1`,
      [userId]
    );
    ...
    await query(
      `UPDATE github_code_config SET scan_status = 'idle', updated_at = NOW() WHERE user_id = $1`,
      [userId]
    );
    throw err;
  }
