...
import type { JiraTicket, TicketComment, AiRecommendation, SimilarTicket } from '../types.js';
...
export async function listTickets(opts: {
  ...
  const { rows } = await query<JiraTicket>(
    `SELECT * FROM jira_tickets ${where} ORDER BY updated_at DESC LIMIT $${params.length - 1} OFFSET $${params.length}`,
    params
  );

  const countResult = await query<{ count: string }>(
    `SELECT COUNT(*)::text as count FROM jira_tickets ${where}`,
    params.slice(0, -2)
  );
...
export async function getTicketByJiraKey(key: string) {
  const { rows } = await query<JiraTicket>('SELECT * FROM jira_tickets WHERE jira_key = $1', [key]);
  return rows[0] || null;
}

export async function getTicketComments(ticketId: string) {
  const { rows } = await query<TicketComment>(
    'SELECT * FROM ticket_comments WHERE ticket_id = $1 ORDER BY created_at_jira ASC',
    [ticketId]
  );
  return rows;
}

export async function getLatestRecommendation(ticketId: string) {
  const { rows } = await query<AiRecommendation>(
    'SELECT * FROM ai_recommendations WHERE ticket_id = $1 ORDER BY created_at DESC LIMIT 1',
    [ticketId]
  );
  return rows[0] || null;
}

export async function findSimilarTickets(ticketId: string, topK = 5): Promise<SimilarTicket[]> {
  ...
  for (const r of results) {
    ...
    await query(
      ...
      [ticketId, r.ticketId, r.similarityScore]
    );
...
export async function analyzeAndRecommend(ticketId: string, userId?: string) {
  const ticket = await getTicketById(ticketId);
  if (!ticket) throw new Error('Ticket not found');
  ...
  const recommendation = await aiClient.generateRecommendation({
    ...
    title: ticket.title,
    description: ticket.description || '',
    similarTickets: similarResults.filter((s: { ticketId: string }) => s.ticketId !== ticketId),
  });

  const similarIds = similarResults
    .map((s: { ticketId: string }) => s.ticketId)
    .filter((id: string) => id !== ticketId);

  const rec = await query<AiRecommendation>(
  ...
  if (userId) {
    await query(
      ...
      [userId, ticketId, JSON.stringify({ confidence: recommendation.confidenceScore })]
    );
  }
...
export async function getAnalyticsSummary() {
  ...
  const [ticketStats, recurring, avgResolution, topCauses, aiUsage] = await Promise.all([
    query<{ total: string; open_count: string; resolved_count: string }>(
      `SELECT
        COUNT(*)::text as total,
  ...
  const recent = await query(
    `SELECT id, jira_key, title, status, priority, updated_at
