...
export async function createSession(userId, ticketId, title) {
    const { rows } = await query(`INSERT INTO chat_sessions (user_id, ticket_id, title)
     VALUES ($1, $2, $3) RETURNING id`, [userId, ticketId || null, title || 'New Chat']);
    return rows[0].id;
}
export async function getSessions(userId) {
    const { rows } = await query(`SELECT cs.*, jt.jira_key, jt.title as ticket_title
...
export async function getSessionMessages(sessionId) {
    const { rows } = await query('SELECT * FROM chat_messages WHERE session_id = $1 ORDER BY created_at ASC', [sessionId]);
    return rows;
}
export async function processChatQuery(opts) {
    ...
    await query(`INSERT INTO chat_messages (session_id, role, content) VALUES ($1, 'user', $2)`, [sessionId, opts.message]);
    ...
    await query(`INSERT INTO chat_messages (session_id, role, content, metadata)
     VALUES ($1, 'assistant', $2, $3)`, [sessionId, result.answer, JSON.stringify({ sources: result.sources })]);
    ...
    await query(`INSERT INTO analytics_events (user_id, event_type, ticket_id, metadata)
     VALUES ($1, 'chat_query', $2, $3)`, [opts.userId, opts.ticketId || null, JSON.stringify({ sessionId })]);
