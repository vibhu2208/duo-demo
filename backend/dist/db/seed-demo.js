...
const demoTickets = JSON.parse(fs.readFileSync(path.join(__dirname, 'demo-tickets.json'), 'utf-8'));
/** Recurring issue cluster → DB category */
function inferCategory(t) {
    const tag = [...t.labels, t.component].join(' ').toLowerCase();
    if (tag.includes('auth') || tag.includes('jwt') || tag.includes('login'))
        return 'authentication';
    if (tag.includes('redis') || tag.includes('timeout'))
        return 'infrastructure';
    if (tag.includes('react') || tag.includes('hydration') || tag.includes('frontend'))
        return 'frontend';
    if (tag.includes('payment') || tag.includes('stripe'))
        return 'billing';
    if (tag.includes('postgresql') || tag.includes('deadlock'))
        return 'database';
    return 'general';
}
function buildEmbeddingResolution(t) {
    return [
...
function buildErrorKeywords(t) {
    const extra = [t.stack_trace, t.error_logs, t.api_endpoint].filter(Boolean);
    return [...new Set([...t.labels, ...extra.flatMap((s) => s.split(/\W+/).filter((w) => w.length > 3))])].slice(0, 20);
}
async function indexTicket(t, ticketId, commentBodies) {
    try {
        ...
        console.warn(`  Index failed for ${t.ticket_id}:`, e.message);
        return false;
    }
}
async function seed() {
    ...
    await pool.query(`INSERT INTO users (email, password_hash, name, role)
     ...
     ON CONFLICT (email) DO NOTHING`, [config.seed.email, hash]);
    const ticketIds = [];
    for (const t of demoTickets) {
        const existing = await pool.query('SELECT id FROM jira_tickets WHERE jira_key = $1', [t.ticket_id]);
        let ticketId;
        const category = inferCategory(t);
        const keywords = buildErrorKeywords(t);
        if (existing.rows[0]) {
            ticketId = existing.rows[0].id;
            await pool.query(`UPDATE jira_tickets SET
         ...
         WHERE id=$1`, [
            ...
            const ins = await pool.query(`INSERT INTO jira_tickets (
        ...
        for (const c of t.comments) {
            await pool.query(`INSERT INTO ticket_comments (ticket_id, author, body) VALUES ($1, $2, $3)`, [ticketId, c.author, c.message]);
        }
        await pool.query('DELETE FROM ai_recommendations WHERE ticket_id = $1', [ticketId]);
        await pool.query(`INSERT INTO ai_recommendations (
    ...
    for (const [a, b, score] of relationshipPairs) {
        ...
        if (keyToId[a] && keyToId[b]) {
            await pool.query(`INSERT INTO ticket_relationships (source_ticket_id, related_ticket_id, similarity_score)
         VALUES ($1, $2, $3), ($2, $1, $3)
         ON CONFLICT (source_ticket_id, related_ticket_id) DO UPDATE SET similarity_score = $3`, [keyToId[a], keyToId[b], score]);
        }
    }
