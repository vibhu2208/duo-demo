...
router.get('/config', async (req, res) => {
    ...
    const lastSync = await query('SELECT last_sync_at, sync_status FROM jira_config WHERE user_id = $1', [req.user.id]);
    const dbRow = await query('SELECT base_url, project_key, deployment_type, sync_filter FROM jira_config WHERE user_id = $1 LIMIT 1', [req.user.id]);
...
router.put('/config', adminOnly, async (req, res) => {
    ...
    await saveJiraConfig(req.user.id, { ...data, apiToken: data.apiToken });
...
router.get('/sync/logs', async (_req, res) => {
    const { rows } = await query('SELECT * FROM sync_logs ORDER BY started_at DESC LIMIT 20');
    res.json({ logs: rows });
});
