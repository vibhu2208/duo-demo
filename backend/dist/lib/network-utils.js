...
export function formatAiServiceError(err) {
    if (axios.isAxiosError(err)) {
        ...
        if (code === 'ECONNRESET' || err.message.toLowerCase().includes('econnreset')) {
            return ('AI service connection was reset (ECONNRESET). On Render free tier the service may be waking up — wait 30s and retry. ' +
...
export async function withRetry(fn, opts = {}) {
    ...
    for (let i = 0; i < attempts; i++) {
        try {
            ...
            lastErr = err;
            if (!isRetryableNetworkError(err) || i === attempts - 1)
                throw err;
